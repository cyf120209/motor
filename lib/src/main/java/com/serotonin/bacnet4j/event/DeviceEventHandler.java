...
//                System.out.println("firePrivateTransfer");
    ...
    public AcknowledgementService fireAckPrivateTransfer(final UnsignedInteger vendorId, final UnsignedInteger serviceNumber,
                                                      final Encodable serviceParameters,Address dev) {
        for (DeviceEventListener l : listeners) {
            try {
                //避免因第一个listener没有对应的方法而返回null，导致没有调用后面的方法
                if(l.privateTransferReceivedComplex(vendorId, serviceNumber, serviceParameters,dev)==null){
                    continue;
                }
                return l.privateTransferReceivedComplex(vendorId, serviceNumber, serviceParameters,dev);
            }
            catch (Throwable e) {
                handleException(l, e);
            }
